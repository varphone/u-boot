...
static void ufs_info_show_portion(void)
{
	...
	UFS_PRINT("UFS %s Gear-%d Rate-%c Lanes-%d\n",((SLOW_MODE == mode)?"Slow" :
				 ((SLOWAUTO_MODE == mode)?"SlowAuto" :
				 ((FAST_MODE == mode)?"Fast" : "FastAuto"))),
...
int ufs_show_desc_info(enum info_show_type type)
{
	...
	if(FALSE == ufs_desc.desc_is_init)
	{
...
void setup_snps_mphy_tc(void)
{
	...
	do {
		if (1 == uic_cmd_read(0x01, 0xD0100000))
			break;
		ufs_waitms(1);
	} while (retry--);
...
int ufs_hc_enable(void)
{
	...
	do {
		...
		i = 0;
		for (;;) {
			if (1 == dwc_ufs_read_reg(UFS_HCE_OFF))
...
/* try to solve the OCS=5 */
	...
	if (0 != uic_cmd_read(0x1, 0xD0AB0000)) {
...
int update_lu_capacity_info(struct partitionDescTable *pdt)
{
	...
	for (i = 0; i < UNIT_DESCS_COUNT; i++) {
		if (NULL == pdt->partitionDescPtr[i])
			continue;
		if (UFS_MAX_LU_CAP == pdt->partitionDescPtr[i]->lu_capacity) {
			if (-1 != max_cap_lu) {
...
unsigned int *ufs_pack_cid(void)
{
	...
	ufs_cid[1] = 0;
	ufs_cid[2] = 0;
	if (UFS_MANUFACTURER_ID_TOSHIBA == dwc_host->manufacturer_id) {
		...
		*  trim the 0x00 byte*/
		...
		snum_buf[10] = 0;
		snum_buf[11] = 0;
	} else if (UFS_MANUFACTURER_ID_SUMSANG == dwc_host->manufacturer_id) {
		/*
		(void)memcpy_s(snum_buf, 12, p, 12);
		*/
	} else if (UFS_MANUFACTURER_ID_HYNIX == dwc_host->manufacturer_id) {
...
*/
	...
	if (DEFAULT_BOOT_LUN != dwc_host->active_lun) {
...
/* 10. Polling IS.UHXS */
	...
	if (1 == getvalue1) {
		...
		if ((2 == getvalue1) && (0 == dwc_ufs_read_reg(UFS_UECDL_OFF)))
