...
static void hinfc610_read_buf(struct mtd_info *mtd, uint8_t *buf, int len)
{
	...
	struct hinfc_host *host = chip->priv;
#ifdef CONFIG_ARCH_MMU
	dcache_inv_range(host->buffer + host->column + host->offset,
			host->buffer + host->column + host->offset + len);
#endif
...
static struct nand_config_info hinfc610_hw_pin_config_table[] = {
				 ...
				 struct nand_config_info *best, int is_hw_auto)
{
	...
	if (pagetype != best->pagetype) {
		if (!isdump++)
			nand_register_dump();
		if (is_hw_auto) {
			printf("!!! warning: Hardware configure pagesize %s," \
	...
	if (IS_RANDOMIZER(nand_dev) && !IS_RANDOMIZER(host)) {
		if (is_hw_auto) {
...
static int hinfc610_nand_init(struct hinfc_host *host, struct nand_chip *chip)
{
	...
	bootmedia = (readl(REG_BASE_SCTL + REG_SYSSTAT) >> 4) & 0x3;
	if (bootmedia == BOOT_FROM_NAND)
