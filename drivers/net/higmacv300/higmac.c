...
static struct higmac_board_info {
	struct higmac_netdev_local higmac_ld;
	enum if_mode phy_intf;
	char *mii_name;
	int phy_addr;
} higmac_board_info[] = { /* MAX_GMAC_NUMS */
	{
		{
			.index		= 0,
...
int eth_rx(void)
{
	...
	if (rx_fq_wr_offset != rx_fq_rd_offset) {
		higmac_writel_bits(ld, rx_fq_rd_offset,
				RX_FQ_WR_ADDR, BITS_RX_FQ_WR_ADDR);
	...
	if (rx_fq_wr_offset == (HIGMAC_HWQ_RX_FQ_DEPTH - 1)<<5)
		rx_fq_wr_offset = 0;
	else
		rx_fq_wr_offset = rx_fq_wr_offset + DESC_SIZE;

	higmac_writel_bits(ld, rx_fq_wr_offset,
			RX_FQ_WR_ADDR, BITS_RX_FQ_WR_ADDR);
	...
	*/
	...
	rx_bq_rd_offset = higmac_readl_bits(ld,
			RX_BQ_RD_ADDR, BITS_RX_BQ_RD_ADDR);
	rx_bq_desc += (rx_bq_rd_offset>>5);

	if (rx_bq_rd_offset == (HIGMAC_HWQ_RX_BQ_DEPTH - 1)<<5)
		rx_bq_rd_offset = 0;
	else
		rx_bq_rd_offset = rx_bq_rd_offset + DESC_SIZE;
	...
	higmac_writel_bits(ld, rx_bq_rd_offset,
			RX_BQ_RD_ADDR, BITS_RX_BQ_RD_ADDR);
...
int eth_send(volatile void *packet, int length)
{
	...
	if (tx_bq_wr_offset == (HIGMAC_HWQ_TX_BQ_DEPTH - 1)<<5)
		tx_bq_wr_offset = 0;
	else
		tx_bq_wr_offset = tx_bq_wr_offset + DESC_SIZE;
	...
	*/
	...
	if (tx_rq_rd_offset == (HIGMAC_HWQ_TX_RQ_DEPTH - 1)<<5)
		tx_rq_rd_offset = 0;
	else
		tx_rq_rd_offset = tx_rq_rd_offset + DESC_SIZE;
