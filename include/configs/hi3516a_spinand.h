...
/* MMU table located after kernel 64M memory */
/* 1G vitual size, use 1024 entries, 4K per table, total 4M size */
#define CONFIG_STB_ADDR	    (0x84000000)

/* ttb table, 4096 entries, 4 bytes per entry, total 4*4096=16k */
#define CONFIG_TTB_ADDR		    (0x84400000)

/* reserved 4M size for uboot,TEXT_BASE=0x84700000 */
#define CONFIG_SNAPSHOT_LOADADDR 0x84800000

#define FASTBOOT_bootloader_OFS  (0)
#define FASTBOOT_bootloader_SZ   (1<<20)

#define FASTBOOT_kernel_OFS      (FASTBOOT_bootloader_OFS \
					+ FASTBOOT_bootloader_SZ)
#define FASTBOOT_kernel_SZ       (3<<20)

#define FASTBOOT_rootfs_OFS      (FASTBOOT_kernel_OFS + FASTBOOT_kernel_SZ)
#define FASTBOOT_rootfs_SZ       (18<<20)

#define FASTBOOT_hibernate_OFS   (FASTBOOT_rootfs_OFS + FASTBOOT_rootfs_SZ)
#define FASTBOOT_hibernate_SZ    (32<<20)
