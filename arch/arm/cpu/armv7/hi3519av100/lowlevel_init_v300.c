...
static void hpm_check(unsigned int* hpm_core, unsigned int* hpm_cpu, unsigned int* hpm_mda, unsigned int hpm_from_otp)
{
    ...
    if(*hpm_core < 300) {*hpm_core = 300; sysboot10.bits.hpm_core_err = 1;}
    if(*hpm_core > 480) {*hpm_core = 480; sysboot10.bits.hpm_core_err = 1;}

    if(*hpm_cpu < 300) {*hpm_cpu = 300; sysboot10.bits.hpm_cpu_err = 1;}
    if(*hpm_cpu > 480) {*hpm_cpu = 480; sysboot10.bits.hpm_cpu_err = 1;}

    if(*hpm_mda < 300) {*hpm_mda = 300; sysboot10.bits.hpm_mda_err = 1;}
    if(*hpm_mda > 480) {*hpm_mda = 480; sysboot10.bits.hpm_mda_err = 1;}
