...
static void ddr_rdbdl_adj(struct training_data *training)
{
	...
	for (i = 0; i < training->ddr_byte_num; i++) {
		...
		rdqs_bdl = (io_read(REG_DDRC_BASE + DDRC_DXNRDQSDLY(i)) & 0x7f)
			+ bdl_offest;
...
static void hw_training(void)
{
	/* gating init */
	unsigned int result;
	io_write(REG_DDRC_BASE + 0x21c, 0);
	io_write(REG_DDRC_BASE + 0x220, 0);
	io_write(REG_DDRC_BASE + 0x29c, 0);
	io_write(REG_DDRC_BASE + 0x2a0, 0);
	io_write(REG_DDRC_BASE + 0x31c, 0);
	io_write(REG_DDRC_BASE + 0x320, 0);
	io_write(REG_DDRC_BASE + 0x39c, 0);
	io_write(REG_DDRC_BASE + 0x3a0, 0);

	io_write(REG_DDRC_BASE + 0x22c, 0);
	io_write(REG_DDRC_BASE + 0x2ac, 0);
	io_write(REG_DDRC_BASE + 0x32c, 0);
	io_write(REG_DDRC_BASE + 0x3ac, 0);
	/* hw read data eye training enable */
	io_write(REG_DDRC_BASE + 0x4, 0x105);

	while ((io_read(REG_DDRC_BASE + 0x4) & 0xffff))
		;
	if (io_read(REG_DDRC_BASE + 0x8) & 0x100) {
		result = 0x4;
...
static void ddr_training_entry(void)
{
	...
	io_write(REG_DDRT_BASE_CRG + DDRT_START_ADDR, 0);
	io_write(DDRT_BURST_TRANSFER + DDRT_START_ADDR, 0x4f);
	io_write(DDRT_BURST_TRANSFER + DDRT_ADDR_NUM, 0xffffffff);
	...
	if (training->hw_training_flag) {
		...
		for (i = 0; i < training->ddr_byte_num; i++) {
			...
			if (((!first_deskew_flag) && (byte_deskew_flag)) ||
				...
				first_deskew_flag = 1;
				rd_dqsbdl = io_read(REG_DDRC_BASE
						+ DDRC_DXNRDQSDLY(i));
