...
static unsigned int g_a_interface_type[VO_DEV_MAX_NUM] = { 0 };

int vo_is_lcd_intf(unsigned int intf_type)
{
...
int vou_drv_check_lcd_sync(vo_dev dev, unsigned int intf_type, unsigned int intf_sync)
{
    if (dev == VO_DEV_DHD0) {
    ...
    if (VO_INTF_LCD_8BIT & intf_type) {
        if (intf_sync != VO_OUTPUT_320x240_60) {
            printf("for LCD 8bit intface,vo%d's intfsync %d illegal!\n", dev, intf_sync);
            return -1;
        }
    }
    ...
    if (VO_INTF_LCD_6BIT & intf_type) {
        if ((intf_sync < VO_OUTPUT_320x240_50) || (intf_sync > VO_OUTPUT_240x320_50)) {
            printf("for LCD 6bit intface,vo%d's intfsync %d illegal!\n", dev, intf_sync);
            return -1;
        }
    }
    ...
    if (VO_INTF_LCD_16BIT & intf_type) {
        if (intf_sync != VO_OUTPUT_240x320_60) {
            printf("for LCD 16bit intface,vo%d's intfsync %d illegal!\n", dev, intf_sync);
            return -1;
        }
    }
    ...
    if (VO_INTF_LCD_18BIT & intf_type) {
        if (intf_sync != VO_OUTPUT_240x320_60) {
            printf("for LCD 18bit intface,vo%d's intfsync %d illegal!\n", dev, intf_sync);
            return -1;
        }
    }
    ...
    if (VO_INTF_LCD_24BIT & intf_type) {
        if (intf_sync != VO_OUTPUT_800x600_50) {
            printf("for LCD 24bit intface,vo%d's intfsync %d illegal!\n", dev, intf_sync);
            return -1;
        }
    }
...
int check_vo_support(unsigned int dev, unsigned int type, unsigned int sync)
{
    ...
    if (dev == VO_DEV_DHD0) {
        ...
        if ((type & ~(VO_INTF_HDMI | VO_INTF_MIPI)) &&
            ...
            printf("for VO %d, only HDMI+MIPI can be used at the same time!\n",
                   dev);
            return -1;
        }
