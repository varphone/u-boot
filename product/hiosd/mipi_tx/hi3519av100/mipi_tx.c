...
static int mipi_tx_set_combo_dev_cfg(combo_dev_cfg_t *p_dev_cfg)
{
    ...
    memcpy(&mipi_tx_dev_ctx, p_dev_cfg, sizeof(combo_dev_cfg_t));
...
int mipi_tx_display(unsigned int vosync)
{
    ...
    */
    s32Ret = mipi_tx_ioctl(HI_MIPI_TX_SET_DEV_CFG, (unsigned long)pstMipiTxConfig,NULL);
    if (0 != s32Ret)
    {
    ...
    */
    s32Ret = mipi_tx_ioctl(HI_MIPI_TX_ENABLE,(unsigned long)0, NULL);
    if (0 != s32Ret)
    {
