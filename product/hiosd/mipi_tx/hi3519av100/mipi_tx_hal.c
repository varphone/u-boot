...
void mipi_tx_drv_set_phy_cfg(combo_dev_cfg_t *dev_cfg)
{
    ...
    /* DATA0~3 TPRE-DELAY */
    data_tpre_delay = (g_actual_phy_data_rate * TPRE_DELAY + 7999) / 8000 - 1;  /* 9 */
    ...
    /* CLK_TLPX */
    clk_tlpx = (g_actual_phy_data_rate * TLPX + 7999) / 8000 - 1;  /* 6 */
    set_phy_reg(CLK_TLPX, clk_tlpx);

    /* CLK_TCLK_PREPARE */
    temp0 = ((g_actual_phy_data_rate * TCLK_PREPARE + 7999) / 8000 - 1 + ((g_actual_phy_data_rate + 799) / 800)
             - ((((g_actual_phy_data_rate * TCLK_PREPARE + 7999) / 8000 - 1 + ((g_actual_phy_data_rate + 799) / 800) + 1) *
                         8000 - 10 * g_actual_phy_data_rate - TCLK_PREPARE * g_actual_phy_data_rate) / 8000));
    if (temp0 > 0) {
            temp1 = temp0;
        } else {
            temp1 = 0;
    ...
    if (((temp1 + 1) * 8000 - 10 * g_actual_phy_data_rate) > 94 * g_actual_phy_data_rate) {
        if (temp0 > 0) {
            clk_tclk_prepare = temp0 - 1;
        } else {
            clk_tclk_prepare = -1;
    ...
    /* CLK_TCLK_ZERO */
    if ((g_actual_phy_data_rate * TCLK_ZERO + 7999) / 8000 > 5) {
        clk_tclk_zero = (g_actual_phy_data_rate * TCLK_ZERO + 7999) / 8000 - 5; /* 19 = 0x13 */
    } else {
        clk_tclk_zero = 0;
    }
    ...
    /* CLK_TCLK_TRAIL */
    clk_tclk_trail = (g_actual_phy_data_rate * TCLK_TRAIL + 7999) / 8000 + 3;  /* 6 */
     ...
     * DATA_THS_TRAIL
     */
    data_tlpx = (g_actual_phy_data_rate * TLPX + 7999) / 8000 - 1;;                /* 6 */

    /* DATA_THS_PREPARE */
    temp0 = ((g_actual_phy_data_rate * THS_PREPARE + 7999) / 8000 - 1 + ((g_actual_phy_data_rate + 799) / 800)
             - ((((g_actual_phy_data_rate * THS_PREPARE + 7999) / 8000 - 1 + ((g_actual_phy_data_rate + 799) / 800) + 1) *
                         8000 - 10 * g_actual_phy_data_rate - THS_PREPARE * g_actual_phy_data_rate) / 8000));
    if (temp0 > 0) {
        temp1 = temp0;
    } else {
        temp1 = 0;
    ...
    (((temp1 + 1) * 8000-10 * g_actual_phy_data_rate) > (85 * g_actual_phy_data_rate + 6 * 1000))) {
    if (temp0 > 0) {
            data_ths_prepare = temp0 - 1;
        } else {
            data_ths_prepare = -1;
            printf("err when calc phy timing \n");
        }
        ...
        if (temp0 > 0) {
            data_ths_prepare = temp0;
        } else {
            data_ths_prepare = 0;
        }
    }

    if ((g_actual_phy_data_rate * THS_ZERO + 7999) / 8000 > 5) {
        data_ths_zero = (g_actual_phy_data_rate * THS_ZERO + 7999) / 8000 - 5; /* 7 */
    } else {
        data_ths_zero = 0;
    }

    data_ths_trail = (g_actual_phy_data_rate * THS_TRAIL + 7999) / 8000 + 3;;      /* 6 */
