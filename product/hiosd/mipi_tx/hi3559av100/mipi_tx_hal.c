...
int mipi_tx_drv_reset_phy(combo_dev_cfg_t *p_dev_cfg)
{
    static int count = 0;
    count++;
#if MIPI_TX_DEBUG
...
void mipi_tx_pll_status_check(combo_dev_cfg_t *p_dev_cfg);
...
void mipi_tx_drv_set_phy_cfg(combo_dev_cfg_t *p_dev_cfg)
{
    ...
    /* pll_set1 */
#ifndef HI_FPGA
    if (p_dev_cfg->phy_data_rate >= 320)
    {
        pll_set1 = p_dev_cfg->phy_data_rate / MIPI_TX_REF_CLK; // 594/27=22
        actual_phy_data_rate = pll_set1 * MIPI_TX_REF_CLK;     // 22*27=594
    }
    else
    {
        pll_set1 = p_dev_cfg->phy_data_rate * 4 / MIPI_TX_REF_CLK;
        actual_phy_data_rate = pll_set1 * MIPI_TX_REF_CLK / 4;
    }
#else
    if (p_dev_cfg->phy_data_rate >= 320)
    {
        pll_set1 = p_dev_cfg->phy_data_rate / MIPI_TX_REF_CLK / 2;
        actual_phy_data_rate = p_dev_cfg->phy_data_rate / MIPI_TX_REF_CLK * MIPI_TX_REF_CLK;
    }
    else
    {
        pll_set1 = p_dev_cfg->phy_data_rate * 4 / MIPI_TX_REF_CLK / 2;
        actual_phy_data_rate = p_dev_cfg->phy_data_rate * 4 / MIPI_TX_REF_CLK  * MIPI_TX_REF_CLK / 4;
    }
#endif
    set_phy_reg(PLL_SET1, pll_set1);
    ...
    /* DATA0~3 TPRE-DELAY */
    data_tpre_delay = (actual_phy_data_rate * TPRE_DELAY + 4000) / 8000;  // 9
    ...
    /* CLK_TLPX */
    clk_tlpx = (actual_phy_data_rate * TLPX + 4000) / 8000;                  // 6
    set_phy_reg(CLK_TLPX, clk_tlpx);

    /* CLK_TCLK_PREPARE */
    clk_tclk_prepare = (actual_phy_data_rate * TCLK_PREPARE + 4000) / 8000;  // 5
    set_phy_reg(CLK_TCLK_PREPARE, clk_tclk_prepare);

    /* CLK_TCLK_ZERO */
    clk_tclk_zero = (actual_phy_data_rate * TCLK_ZERO + 4000) / 8000;        // 19 = 0x13
    set_phy_reg(CLK_TCLK_ZERO, clk_tclk_zero);

    /* CLK_TCLK_TRAIL */
    clk_tclk_trail = (actual_phy_data_rate * TCLK_TRAIL + 4000) / 8000;      // 6
    ...
    /*------------------
       ...
       DATA_THS_TRAIL
    ------------------*/
    data_tlpx = (actual_phy_data_rate * TLPX + 4000) / 8000;                  // 6
    data_ths_prepare = (actual_phy_data_rate * THS_PREPARE + 4000) / 8000;    // 5
    data_ths_zero = (actual_phy_data_rate * THS_ZERO + 4000) / 8000;          // 7
    data_ths_trail = (actual_phy_data_rate * THS_TRAIL + 4000) / 8000;        // 6
...
static void set_video_mode_cfg(combo_dev_cfg_t *p_dev_cfg)
{
    ...
    if (NON_BRUST_MODE_SYNC_PULSES == p_dev_cfg->video_mode)
    {
        video_mode = 0;
    }
    else if (NON_BRUST_MODE_SYNC_EVENTS == p_dev_cfg->video_mode)
    {
        video_mode = 1;
    }
    else
...
static void set_timing_config(combo_dev_cfg_t *p_dev_cfg)
{
    ...
    {
        printf("p_dev_cfg->pixel_clk is 0, illegal.\n");
        return;
    }
...
void mipi_tx_drv_set_controller_cfg(combo_dev_cfg_t *p_dev_cfg)
{
    ...
    udelay(100);
    mipi_tx_pll_status_check(p_dev_cfg);
...
void mipi_tx_drv_set_cmd_info(cmd_info_t *cmd_info)
{
    ...
    if (NULL != cmd_info->cmd)
    {
        ...
        if (memcpy(cmd, cmd_info->cmd, cmd_info->cmd_size))
        {
            free(cmd);
            return;
        }
...
static int mipi_tx_drv_reg_init(void)
{
    if (!mipi_tx_regs_va)
    {
        mipi_tx_regs_va = (mipi_tx_regs_type_t *)IO_ADDRESS(MIPI_TX_REGS_ADDR);
...
{
    unsigned int u32phy_lock = 0x0;

    u32phy_lock = mipi_tx_regs_va->PHY_STATUS.bits.phy_lock;
    return u32phy_lock;
}

int hal_phy_getphy_statusphy_stopstateXlane(unsigned int x, unsigned int * phy_stopstateXlane)
{
    switch(x)
    {
        // 0lane
        case 0:
            *phy_stopstateXlane = mipi_tx_regs_va->PHY_STATUS.bits.phy_stopstate0lane;
            break;
        // 1lane
        case 1:
            *phy_stopstateXlane = mipi_tx_regs_va->PHY_STATUS.bits.phy_stopstate1lane;
            break;
        // 2lane
        case 2:
            *phy_stopstateXlane = mipi_tx_regs_va->PHY_STATUS.bits.phy_stopstate2lane;
            break;
        // 3lane
        case 3:
            *phy_stopstateXlane = mipi_tx_regs_va->PHY_STATUS.bits.phy_stopstate3lane;
            break;
        default:
            // do nothing.
            break;
...
int hal_phy_getphy_statusphy_ulpsactivenotXlane(unsigned int x, unsigned int * phy_ulpsactivenotXlane)
{
    switch(x)
    {
        // 0lane
        case 0:
            *phy_ulpsactivenotXlane = mipi_tx_regs_va->PHY_STATUS.bits.phy_ulpsactivenot0lane;
            break;
        // 1lane
        case 1:
            *phy_ulpsactivenotXlane = mipi_tx_regs_va->PHY_STATUS.bits.phy_ulpsactivenot1lane;
            break;
        // 2lane
        case 2:
            *phy_ulpsactivenotXlane = mipi_tx_regs_va->PHY_STATUS.bits.phy_ulpsactivenot2lane;
            break;
        // 3lane
        case 3:
            *phy_ulpsactivenotXlane = mipi_tx_regs_va->PHY_STATUS.bits.phy_ulpsactivenot3lane;
            break;
        default:
            // do nothing.
            break;
...
int MIPI_TX_DRV_GetPhyStatusPhy_stopstateXlane(unsigned int xlane)
{
    unsigned int u32phy_stopstateXlane = 0;

    hal_phy_getphy_statusphy_stopstateXlane(xlane, &u32phy_stopstateXlane);
    return u32phy_stopstateXlane;
}


unsigned int MIPI_TX_DRV_GetPhyStatusPhy_ulpsactivenotXlane(unsigned int xlane)
{
    unsigned int u32phy_ulpsactivenotXlane = 0;

    hal_phy_getphy_statusphy_ulpsactivenotXlane(xlane, &u32phy_ulpsactivenotXlane);
    return u32phy_ulpsactivenotXlane;
}

void MIPI_TX_DRV_SetUlpsMode(void)
{
    hal_phy_setphy_ulps_ctrlphy_txrequlpslan(0x1);
}

void MIPI_TX_DRV_ExitUlpsMode(void)
{
    hal_phy_setphy_ulps_ctrlphy_txrequlpslan(0x1);
...
unsigned int MIPI_TX_DRV_GetPhyStatusPhy_stopstateclklane(void)
{
...
unsigned int MIPI_TX_DRV_GetPhyStatus(void)
{
    unsigned int u32phy_status = 0;

    hal_phy_getphy_status(&u32phy_status);
    return u32phy_status;
}
...
step2 :
    ...
    while(i<trytimes)
    {
        if(1 == mipi_tx_drv_get_phy_status_phy_lock())
        {
            // check phy lock
            break;
        }
    ...
    {
        // reset phy
        count = mipi_tx_drv_reset_phy(p_dev_cfg);
        if(MIPI_TX_MAX_RESET_PHY_TIMES == count)
        {
            goto exit;
        }
        // goto step2
    ...
    while(i<trytimes)
    {
        if( 1 == MIPI_TX_DRV_GetPhyStatusPhy_stopstateXlane(0) &&
            1 == MIPI_TX_DRV_GetPhyStatusPhy_stopstateXlane(1) &&
            1 == MIPI_TX_DRV_GetPhyStatusPhy_stopstateXlane(2) &&
            1 == MIPI_TX_DRV_GetPhyStatusPhy_stopstateXlane(3))
        {
    ...
    // step 5
    MIPI_TX_DRV_SetUlpsMode();

    // step 6
    i=0;
    while(i<trytimes)
    {
        if( 0 == MIPI_TX_DRV_GetPhyStatusPhy_ulpsactivenotXlane(0) &&
            0 == MIPI_TX_DRV_GetPhyStatusPhy_ulpsactivenotXlane(1) &&
            0 == MIPI_TX_DRV_GetPhyStatusPhy_ulpsactivenotXlane(2) &&
            0 == MIPI_TX_DRV_GetPhyStatusPhy_ulpsactivenotXlane(3))
        {
    ...
    MIPI_TX_DRV_ExitUlpsMode();

    // step 8
    i=0;
    while(i<trytimes)
    {
        if( 1 == MIPI_TX_DRV_GetPhyStatusPhy_stopstateclklane() &&
            1 == MIPI_TX_DRV_GetPhyStatusPhy_ulpsactivenotXlane(0) &&
            1 == MIPI_TX_DRV_GetPhyStatusPhy_ulpsactivenotXlane(1) &&
            1 == MIPI_TX_DRV_GetPhyStatusPhy_ulpsactivenotXlane(2) &&
            1 == MIPI_TX_DRV_GetPhyStatusPhy_ulpsactivenotXlane(3))
        {
