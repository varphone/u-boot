...
HI_S32 HAL_HDMI_PhyInit(HDMI_HAL_INIT_S *pstHalInit)
{
    ...
    {
#ifndef HDMI_BUILD_IN_BOOT
        if (pstHalInit->pcPhyAddr == HI_NULL && pstHalInit->pcBaseAddr != HI_NULL) {
            HDMI_HDMITX_PHY_reg_Init((pstHalInit->pcBaseAddr + HDMI_TX_BASE_ADDR_DPHY));
        } else if (pstHalInit->pcPhyAddr != HI_NULL) {
            HDMI_HDMITX_PHY_reg_Init(pstHalInit->pcPhyAddr);
        } else {
            HDMI_ERR("null pointer!\n");
            return HI_FAILURE;
        }
