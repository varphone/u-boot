...
static HI_U32 g_au32BgColor[2] = {0x00FF, 0xFF};
...
int start_vo(unsigned int dev, unsigned int type, unsigned int sync)
{
    if (g_bInited == HI_FALSE)
    {
        ...
        SYS_HAL_SetVoLowPowerCtrl(HI_TRUE);
    ...
    unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
        ...
        VO_INTF_BT1120 == enVoIntfType)
    {
        ...
        HAL_LAYER_SetCscEn(enLayer, HI_FALSE);
    }
    else
    {
        // do yuv to rgb.
        ...
        stCscCoefParam.csc_scale2p  = GFX_CSC_SCALE;
        stCscCoefParam.csc_clip_min = GFX_CSC_CLIP_MIN;
        stCscCoefParam.csc_clip_max = GFX_CSC_CLIP_MAX;

        // do rgb to yuv.
        VOU_DRV_SetCscCoef(enHalLayer, &stCsc, &stCscCoefParam);
    ...
    HAL_VIDEO_HFIR_SetCkGtEn(layer, HI_TRUE);
    ...
    unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
        ...
        (VO_INTF_BT1120 | enVoIntfType))
    {
        ...
        stGfxCsc.enCscMatrix  = VO_CSC_MATRIX_RGB_TO_BT601_TV;
        stGfxCsc.u32Luma      = 50;
        ...
        // do rgb to yuv.
        VOU_DRV_SetCscCoef(enHalLayer, &stGfxCsc,&stCscCoefParam);
