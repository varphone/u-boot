...
HI_VOID VOU_DRV_DefLayerBindDev(HI_VOID)
{
    ...
    HAL_CBM_SetCbmMixerPrio(HAL_DISP_LAYER_GFX0, VOU_MIX_PRIO1, HAL_CBMMIX1);
...
HI_VOID VOU_DRV_SetDevClk(VO_DEV VoDev)
{
    ...
    HI_U32 u32HdmiclkDiv   = 0x0;
    HI_U32 u32ClkDiv       = 0x0;
    ...
    switch (g_stHalDevCfg[VoDev].enOutSync)
    {
        ...
        {
            u32VdpOutClkSel = 0x6;
            u32LcdMclkDiv   = 0x3a9cc2;//0x39cc92;
            SYS_HAL_LcdMclkDiv(u32LcdMclkDiv);
            u32HdmiclkDiv  = 0x0;
            break;
        }
    ...
    SYS_HAL_SetVoPllPostdiv2(u32Postdiv1);
    SYS_HAL_SetVoPllPostdiv1(u32Postdiv2);
    return ;
}
...
//    VO_CSC_S                 stVgaCSC;
    ...
    if (VO_INTF_HDMI & pstHalDevCfg->enIntfType)
    {
        ...
        SYS_HAL_VouHd0DivMode(u32Hd0DivMod);   // [12:13] VDP HD0 div mode
...
HI_VOID VOU_DRV_DefaultSetting(HI_VOID)
{
    ...
    HAL_LAYER_SetLayerGalpha(HAL_DISP_LAYER_VHD0, 255);

    /* outstanding */
    VOU_DRV_LayerEnable(HAL_DISP_LAYER_VHD0, HI_FALSE);
