...
HI_S32 PKCS_PUB_ENC(HI_UNF_CIPHER_RSA_ENC_SCHEME_E enScheme, HI_U8 *pu8Expect)
{
    ...
    if(enScheme == HI_UNF_CIPHER_RSA_ENC_SCHEME_NO_PADDING)
    {
        if(memcmp(u8Dec, NO_PADDING, 256) != 0)
        {
            ...
            Cipher_printBuffer("enc", u8Enc, u32OutLen);
            Cipher_printBuffer("dec", u8Dec, u32OutLen);
           ...
           const unsigned char u8Src[16], const unsigned char u8Dst[16])
{
    ...
    //Cipher_printBuffer("ENCRYPT", (HI_U8*)output, 16);
    ...
    //Cipher_printBuffer("DECRYPT", (HI_U8*)output, 16);
...
static int klad_test(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    Cipher_printBuffer("clear text:", (HI_U8*)u8InBuf, u32TestDataLen);
    ...
    //Cipher_printBuffer("encrypted text:", (HI_U8*)u8OutBuf, u32TestDataLen);
   ...
   /* For decrypt */
    ...
    //Cipher_printBuffer("before decrypt:", (HI_U8*)u8InBuf, u32TestDataLen);
    ...
    //Cipher_printBuffer("decrypted text:", (HI_U8*)u8OutBuf, u32TestDataLen);
    /* compare */
